...
eta, rho, v0 = np.loadtxt('new_metrics/combined_labeled_metrics_trial1.csv', delimiter=',', usecols=range(0, 3), unpack=True)

orders, binders, susceps, labels = [], [], [], []
for i in range(1, 6):
    o, b, s, l = np.loadtxt(f'new_metrics/combined_labeled_metrics_trial{i}.csv', delimiter=',', unpack=True)
    orders.append(o)
    binders.append(b)
    susceps.append(s)
    labels.append(l)
...
df2.to_csv('new_metrics/combinedOverallMetrics.csv', index=False)
