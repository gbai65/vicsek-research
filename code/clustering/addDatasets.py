...
eta, rho, v0 = np.loadtxt('overall_metrics.txt', delimiter=',', usecols=range(0, 3), unpack=True)

orders, binders, susceps, labels = [], [], [], []
for i in range(1, 6):
    o, b, s, l = np.loadtxt(f'labeled_metrics_trial{i}.csv', delimiter=',', unpack=True)
    orders.append(o)
    binders.append(b)
    susceps.append(s)
    labels.append(l)
...
df1.to_csv('labeledDataset.csv', index=False)
...
df2.to_csv('fullDataset.csv', index=False)
