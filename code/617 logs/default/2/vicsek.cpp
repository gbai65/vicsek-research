...
void save_state(
    ...
    // 1. Prepare shapes for all datasets
    ...
    // 3. Save everything sequentially using "w" for the first file initialization,
    // but tightly managed. Better yet, create a clean map if your cnpy version supports it.
    // If using basic cnpy, use standard file locks or avoid appending completely:
    
    // Clear any existing corrupted file first
    std::filesystem::remove(filename); 

    // Write arrays sequentially to the same file safely
...
int main(int argc, char* argv[]) {
    ...
    std::string final_file = directoryName + "/final_state.npz";
    save_state(final_file, P, pos, orient, P.NSTEPS);

    auto end_time = chrono::steady_clock::now();
    double total_elapsed = chrono::duration<double>(end_time - start).count();
    outFile << "Simulation complete. Total time: " << total_elapsed << "s\n";
